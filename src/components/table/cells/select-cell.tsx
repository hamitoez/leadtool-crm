...
import { CellProps, CellValue, JsonValue } from "@/types/table";
...
export const SelectCell = React.memo<CellProps>(
          ...
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              <Button
                variant="ghost"
                size="sm"
                className="h-6 w-full justify-start p-0 font-normal hover:bg-transparent"
              >
                {selectedOption ? (
                  <Badge className={cn("px-1.5 py-0 text-[10px]", selectedOption.color)}>
                    {selectedOption.label}
                  </Badge>
                ) : (
                  <span className="text-xs text-muted-foreground/50"></span>
                )}
                <ChevronDown className="ml-auto h-3 w-3 opacity-30" />
              </Button>
            </PopoverTrigger>
